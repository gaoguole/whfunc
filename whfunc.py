...
def 根据合约名字列表和周期查询出数据字典(合约名称列表,周期):
    d={}
...
def numpy_to_pandas(数据):
    return pd.DataFrame(数据,columns=["datetime",'open','high','low','close',"volume","open_oi","close_oi"])




def 根据限定时间查到涉及到的具体合约(起始时间,结束时间,查询列表,历史数据文件夹):
    l=[]
    起始时间戳=time.mktime(time.strptime(起始时间,"%Y-%m-%d"))*1e9
    结束时间戳=time.mktime(time.strptime(结束时间,"%Y-%m-%d"))*1e9

    for x in 查询列表:
        data=pd.read_csv(历史数据文件夹+"\\"+x+'.csv')
        for y in range(len(data)):
            #print(起始时间戳,data.datetime.iloc[x],结束时间戳)
            if 起始时间戳<data.datetime.iloc[y]<结束时间戳:
...
def 查询品种主力的月份(api,品种):
    ...
    t3=time.time()
    订阅数据合约字典={}
    # 订阅数据合约字典={  x:api.get_kline_serial(x,60*60*24,2000)  for x in 品种相关合约}
    for x in 品种相关合约:
        a=订阅异常(api,x)
        if type(a) !=type(0):
            订阅数据合约字典[x]=a
    ...
    t4=time.time()
    订阅数据合约新字典={   x:{ 订阅数据合约字典[x].datetime.iloc[y]  : 订阅数据合约字典[x].volume.iloc[y] for y in range(len(订阅数据合约字典[x]))}      for x in 订阅数据合约字典}
    t5=time.time()
    ...
    for x in range(0,2000,10):
        if 订阅主连.datetime.iloc[x]:
            ...
            成交量=订阅主连.volume.iloc[x]
            for y in 订阅数据合约新字典:
                if 日期 in 订阅数据合约新字典[y]:
                    if 订阅数据合约新字典[y][日期]==成交量:
                        l.append(y)
                        break
            else:
                l.append("")
...
def WH_获取时间对应id(行情,时间点):
    ...
    时间点时间戳=time.mktime(a)*1e9
    for x in range(len(行情)):
        if 行情.datetime.iloc[x]>=时间点时间戳:
            return 行情.id.iloc[x]
